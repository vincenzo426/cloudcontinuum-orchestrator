...
"""
Training script con MaskablePPO e TensorBoard Avanzato
VERSIONE 2.7 - FIX LOGGER & SCHEDULE

CHANGELOG v2.7:
- Fix AttributeError: Configurazione esplicita del Logger (sb3.common.logger.configure)
- Fix DeprecationWarning: Sostituito get_schedule_fn con lambda functions
- Include tutti i fix precedenti (Buffer, Activation, Masking)
"""
...
# FIX: Importa configure per gestire manualmente il logger
...
class TensorboardCallback(BaseCallback):
    """
    Callback custom per loggare metriche specifiche dell'ambiente su TensorBoard.
    """
    def __init__(self, verbose=0):
        super().__init__(verbose)

    def _on_step(self) -> bool:
        # Safety check: se il logger non è pronto, salta questo step
...
# Wrapper function per l'Action Masker
...
def train_stage(
    ...
    if model is None:
        print(f"Initializing MaskablePPO (Logs: {log_path})...")
        model = MaskablePPO(
            "MlpPolicy",
            env,
            verbose=1,
        ...
        print(f"Continuing training (Logs: {log_path})...")
        model.set_env(env)
        
        # FIX LOGGER: Configurazione esplicita invece di set_logger(None)
        # Questo garantisce che il logger esista prima che partano le callback
        new_logger = configure(log_path, ["stdout", "tensorboard"])
        model.set_logger(new_logger)
        
        # FIX SCHEDULE: Usa lambda invece di get_schedule_fn (deprecated)
        ...
        # FIX BUFFER: MaskableRolloutBuffer
...
def train_curriculum(save_dir: str = "./models") -> MaskablePPO:
    print("\n" + "=" * 70)
    print(" " * 20 + "CURRICULUM LEARNING (MASKABLE + TENSORBOARD)")
    print("=" * 70)
...
def evaluate_final_model(model_path: str, difficulty: str = "hard") -> Dict:
    print("\n" + "=" * 70)
    print(" " * 20 + "FINAL MODEL EVALUATION (MASKED)")
    print("=" * 70)
...
def main():
    ...
    if args.eval_only:
        ...
        train_curriculum(save_dir=args.save_dir)
        evaluate_final_model(f"{args.save_dir}/stage3_hard/best_model/best_model.zip")
